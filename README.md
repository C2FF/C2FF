...
6. **TERM** - two working modes: **PERSO** (a private shell per member - nobody writes in yours; remote members run inside the same disposable Docker sandbox, so they only ever see the group working directory, never the host's files) and **GROUP** (shared command cards: every command is signed with its author's handle, auto-classified with a short type description - HTTP request, port scan, DNS enum... - outputs stay collapsed behind a *voir* button with a size hint, truncation caps both sides, anti-flood cooldown between commands, Ctrl+C kills your own running command; each command runs in a disposable Docker sandbox - image `c2ff-sandbox`, only the group working directory is mounted read-write, caps dropped, memory/PIDs/CPU quotas, a `sandbox`/`hote` pill shows where it ran, falls back to a plain host shell without Docker - `config.json` → `group_terminal.sandbox`); co-admins and the admin also get a read-only **terminal** button next to each member in TEAM to watch that member's private terminal (cards are also served over a polling fallback so the member display survives tunnels that buffer SSE)
