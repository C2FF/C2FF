...
6. **TERM** - two working modes: **SOLO** (private real shell, admin) and **GROUP** (shared command cards: every command is signed with its author's handle, auto-classified with a short type description - HTTP request, port scan, DNS enum... - outputs stay collapsed behind a *voir* button with a size hint, truncation caps both sides, anti-flood cooldown between commands, Ctrl+C kills your own running command; each command runs in a disposable Docker sandbox - image `c2ff-sandbox` (build it with `install.sh` or see `docs/`), only the group working directory is mounted read-write, hunter tooling is mounted read-only, caps dropped, memory/PIDs/CPU quotas - the card shows a `sandbox`/`hote` pill so members know where it ran; falls back to a plain host shell if Docker is unavailable, configurable in `config.json` → `group_terminal.sandbox`)
